"""Shared runner for roundabout middle-density simple-interaction attack scripts."""
...
from cosim_utils.attack_manager import (
    FalseInformationInjectionAttack,
    ObstacleGhostVehicleAttack,
    ReplayAttack,
    StaticGhostVehicleAttack,
    SybilAttack,
    V2XAttackManager,
    assign_attack_vehicle_ids,
)
from cosim_utils.dataset_saver import DatasetSaver
from cosim_utils.helpers import is_port_open, load_scenario, scenario_lane_paths_for_trips, scenario_trip_specs, set_random_seed
...
REPLAY_RECORD_PATH = DATASET_ROOT / "replay_records/roundabout/roundabout_scenario_01/south_to_west.jsonl"
CAMERA_HEIGHT = 150
RANDOM_SEED = 42
...
MAX_STEPS = 900
WARMUP_STEPS = 0
SAME_ORIGIN_DEPARTURE_GAP_TICKS = 20
SAME_ORIGIN_SPAWN_CLEARANCE_M = 18.0
...
ENTRY_MOVEMENTS = {
    "south": {
        "right": "south_to_east",
        "left": "south_to_west",
        "straight": "south_to_north",
    },
    "east": {
        "right": "east_to_north",
        "left": "east_to_south",
        "straight": "east_to_west",
    },
    "north": {
        "right": "north_to_west",
        "left": "north_to_east",
        "straight": "north_to_south",
    },
    "west": {
        "right": "west_to_south",
        "left": "west_to_north",
        "straight": "west_to_east",
    },
}
MANEUVER_SEQUENCE = ("right", "left", "straight", "right", "left")


def movement_names_for_entry(entry_direction):
    movements = ENTRY_MOVEMENTS[entry_direction]
    return tuple(movements[maneuver] for maneuver in MANEUVER_SEQUENCE)
...
def build_attack(attack_kind, controller_vids, run_start_tick, scenario):
    start_tick = run_start_tick + (30 if attack_kind == "replay" else ATTACK_START_TICK)
    primary_vid = controller_vids[0]
    if attack_kind == "false_information_injection":
        attack = FalseInformationInjectionAttack(
            attacker_vehicle_id=primary_vid,
    ...
    elif attack_kind == "replay":
        replay_count = count_jsonl_records(REPLAY_RECORD_PATH)
        attack = ReplayAttack(
            replay_name="roundabout_south_to_west",
            replay_path=REPLAY_RECORD_PATH,
            replay_sender_id=None,
            start_tick=start_tick,
            end_tick=start_tick + replay_count - 1,
            loop=False,
            use_limiter=False,
        )
    elif attack_kind == "static_ghost_vehicle":
        attack = StaticGhostVehicleAttack(
            target_vehicle_id=primary_vid,
            ghost_id=None,
            scenario_type="roundabout",
            merging_point_center_xy_carla=scenario.get("intersection_center_xy_carla"),
            speed_mps=0.0,
            start_tick=start_tick,
            end_tick=start_tick + 220,
        )
    elif attack_kind == "obstacle_ghost_vehicle":
...
def generate_trips_with_gaps(cosim_client, trip_specs, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state):
    ...
    for vid, road_from, road_to, _movement_name in trip_specs:
        while (
...
def _origin_ready_for_next_departure(cosim_client, road_from, last_generated_tick_by_origin, generated_vids_by_origin):
    ...
    if last_tick is not None and cosim_client.current_tick - last_tick < SAME_ORIGIN_DEPARTURE_GAP_TICKS:
        return False
    ...
    for previous_vid in generated_vids_by_origin.get(road_from, []):
        ...
        if previous_vehicle.get_location().distance(spawn_loc) < SAME_ORIGIN_SPAWN_CLEARANCE_M:
            return False

    return True


def run_roundabout_attack(case_name, attack_kind, entry_direction):
    return run_roundabout_attack_movements(
        case_name,
        attack_kind,
        movement_names_for_entry(entry_direction),
        INTERACTION_COMPLEXITY,
    )


def run_roundabout_attack_movements(case_name, attack_kind, movement_names, interaction_complexity):
    ...
    try:
        ...
        attack = build_attack(attack_kind, controller_vids, run_start_tick, scenario)
