"""Straight-road scenario 01 middle-density complicated interaction: six negative-direction vehicles with false information injection attack and overtaking lane changes enabled."""
...
CASE_NAME = "NegativeDirection_6_ComplicatedLaneChange_FalseInformationInjection"
SCENARIO_PATH = ROOT_DIR / "V2V-Attack-Dataset/scenarios/straight_road/straight_road_scenario_01.yaml"
DATASET_ROOT = ROOT_DIR / "V2V-Attack-Dataset"
MOVEMENT_NAMES = ("negative", "negative", "negative", "negative", "negative", "negative")
LANE_PATHS = {
    1: ["-38_3", "-12_3", "-34_3"],
    2: ["-38_3", "-12_3", "-34_3"],
    3: ["-38_4", "-12_4", "-34_4"],
...
def build_attack(controller_vids, run_start_tick):
    ...
    attack = FalseInformationInjectionAttack(
        attacker_vehicle_id=controller_vids[0],
...
def configure_run(scenario):
    ...
    config.controller_kwargs = {"route_project_to_carla_map": False, "enable_overtake_lane_change": True}
...
def run_one_step(cosim_client, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state):
    ...
    cosim_client.set_follow_vehicle_overhead_camera(FOLLOW_CAMERA_VEHICLE_ID, height=CAMERA_HEIGHT)
...
def trip_lane_order(trip_spec):
    vid = trip_spec[0]
    lane_key = tuple(LANE_PATHS.get(vid, [trip_spec[1]]))
    same_lane_vids = [other_vid for other_vid, lane_path in LANE_PATHS.items() if tuple(lane_path) == lane_key]
    same_lane_vids.sort()
    return same_lane_vids.index(vid), lane_key, vid
...
def generate_trips_with_gaps(cosim_client, trip_specs, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state):
    ...
    for vid, road_from, road_to, movement_name in sorted(trip_specs, key=trip_lane_order):
...
def main():
    ...
    try:
        ...
        camera_x, camera_y = scenario.get("road_center_xy_carla", scenario.get("intersection_center_xy_carla", [0, 0]))
