"""On/off-ramp scenario 01 middle-density complicated interaction: six vehicles with false information injection attack."""
...
CASE_NAME = "OnOffRamp_6_FalseInformationInjection"
...
def build_attack(controller_vids, run_start_tick, scenario):
    ...
    attacker_vid = OFF_RAMP_LANE4_VID
    if attacker_vid not in controller_vids:
        raise ValueError(f"Configured attacker vehicle {attacker_vid} is not in controller_vids={controller_vids}")
...
def generate_one_trip(cosim_client, trip_spec, last_generated_tick_by_start_lane, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state):
...
def generate_trips_with_gaps(cosim_client, trip_specs, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state):
    ...
    for vid in (3, 4, 5, 6):
        generate_one_trip(cosim_client, trip_by_vid[vid], last_generated_tick_by_start_lane, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state)
    wait_until_relative_tick(cosim_client, FIRST_ON_RAMP_DEPARTURE_TICK, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state)
    generate_one_trip(cosim_client, trip_by_vid[1], last_generated_tick_by_start_lane, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state)
    wait_until_relative_tick(cosim_client, SECOND_ON_RAMP_DEPARTURE_TICK, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state)
    generate_one_trip(cosim_client, trip_by_vid[2], last_generated_tick_by_start_lane, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state)
