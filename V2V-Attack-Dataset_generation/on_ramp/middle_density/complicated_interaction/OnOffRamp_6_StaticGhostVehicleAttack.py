"""On/off-ramp scenario 01 middle-density complicated interaction: six vehicles with static ghost vehicle attack."""
...
CASE_NAME = "OnOffRamp_6_StaticGhostVehicleAttack"
...
def build_attack(controller_vids, run_start_tick, scenario):
    ...
    target_vid = OFF_RAMP_LANE4_VID
    if target_vid not in controller_vids:
        raise ValueError(f"Configured target vehicle {target_vid} is not in controller_vids={controller_vids}")
    diverging_point = scenario.get("diverging_point_center_xy_carla", OFF_RAMP_POINT_CENTER_XY_CARLA)
    attack = StaticGhostVehicleAttack(
        target_vehicle_id=target_vid,
        ghost_id=None,
        scenario_type="diverging",
        diverging_point_center_xy_carla=diverging_point,
        speed_mps=0.0,
