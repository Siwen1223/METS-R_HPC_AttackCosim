"""On-ramp scenario 01 middle-density moderately complex interaction: six vehicles with false information injection attack."""
...
from cosim_utils.attack_manager import FalseInformationInjectionAttack, V2XAttackManager, assign_attack_vehicle_ids
...
CASE_NAME = "OnRamp_6_FalseInformationInjection"
SCENARIO_PATH = ROOT_DIR / "V2V-Attack-Dataset/scenarios/on_ramp/on_ramp_scenario_01.yaml"
DATASET_ROOT = ROOT_DIR / "V2V-Attack-Dataset"
MOVEMENT_NAMES = ("on_ramp", "mainline", "mainline", "mainline", "mainline", "mainline")
LANE_PATHS = {
    1: ["2_2", "45_2", "-29#0_2"],
    2: ["-16_2", "-25_2", "-28_2", "-29#0_2"],
    3: ["-16_2", "-25_2", "-28_2", "-29#0_2"],
    4: ["-16_2", "-25_2", "-28_2", "-29#0_2"],
    5: ["-16_3", "-25_3", "-28_3", "-29#0_3"],
    6: ["-16_3", "-25_3", "-28_3", "-29#0_3"],
}
MAINLINE_FIRST_INDEX = 1
CAMERA_HEIGHT = 180
RANDOM_SEED = 42
...
MAX_STEPS = 500
WARMUP_STEPS = 0
SAME_ROUTE_DEPARTURE_GAP_TICKS = 30
RAMP_DEPARTURE_DELAY_TICKS = 50
...
def build_attack(controller_vids, run_start_tick, scenario):
    start_tick = run_start_tick + ATTACK_START_TICK
    attacker_vid = controller_vids[MAINLINE_FIRST_INDEX]
    attack = FalseInformationInjectionAttack(
        attacker_vehicle_id=attacker_vid,
        forward_offset_m=18.0,
        lateral_offset_m=0.0,
        speed_bias_mps=0.0,
        start_tick=start_tick,
        end_tick=start_tick + 220,
    )
...
def generate_one_trip(cosim_client, trip_spec, last_generated_tick_by_lane, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state):
    vid, road_from, road_to, movement_name = trip_spec
    lane_key = tuple(LANE_PATHS.get(vid, [road_from]))
    while (
        lane_key in last_generated_tick_by_lane
        and cosim_client.current_tick - last_generated_tick_by_lane[lane_key] < SAME_ROUTE_DEPARTURE_GAP_TICKS
    ):
    ...
    last_generated_tick_by_lane[lane_key] = cosim_client.current_tick


def wait_until_relative_tick(cosim_client, target_relative_tick, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state):
...
def generate_trips_with_gaps(cosim_client, trip_specs, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state):
    trip_by_vid = {vid: spec for spec in trip_specs for vid in [spec[0]]}
    last_generated_tick_by_lane = {}
    for vid in (2, 5, 3):
        generate_one_trip(cosim_client, trip_by_vid[vid], last_generated_tick_by_lane, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state)
    wait_until_relative_tick(cosim_client, RAMP_DEPARTURE_DELAY_TICKS, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state)
    generate_one_trip(cosim_client, trip_by_vid[1], last_generated_tick_by_lane, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state)
    for vid in (6, 4):
        generate_one_trip(cosim_client, trip_by_vid[vid], last_generated_tick_by_lane, data_saver, attacks, attack, controller_vids, run_start_tick, dt, state)
...
def main():
    ...
    try:
        ...
        attack = build_attack(controller_vids, run_start_tick, scenario)
        ...
        camera_x, camera_y = scenario.get("merging_point_center_xy_carla", [0, 0])
