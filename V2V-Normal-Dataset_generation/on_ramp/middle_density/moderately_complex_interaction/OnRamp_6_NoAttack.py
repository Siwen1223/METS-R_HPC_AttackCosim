"""On-ramp scenario 01 middle-density moderately complex interaction: six vehicles without attack."""
...
def run_one_step(cosim_client, data_saver, controller_vids, run_start_tick, dt, state):
    predicted_tick = cosim_client.current_tick + 1
    relative_tick = predicted_tick - run_start_tick
    relative_time = relative_tick * dt

    step_result = cosim_client.step(phase="normal")
...
def generate_one_trip(cosim_client, trip_spec, last_generated_tick_by_lane, data_saver, controller_vids, run_start_tick, dt, state):
...
def generate_trips_with_gaps(cosim_client, trip_specs, data_saver, controller_vids, run_start_tick, dt, state):
    ...
    for vid in (2, 5, 3):
        generate_one_trip(cosim_client, trip_by_vid[vid], last_generated_tick_by_lane, data_saver, controller_vids, run_start_tick, dt, state)
    wait_until_relative_tick(cosim_client, RAMP_DEPARTURE_DELAY_TICKS, data_saver, controller_vids, run_start_tick, dt, state)
    generate_one_trip(cosim_client, trip_by_vid[1], last_generated_tick_by_lane, data_saver, controller_vids, run_start_tick, dt, state)
    for vid in (6, 4):
        generate_one_trip(cosim_client, trip_by_vid[vid], last_generated_tick_by_lane, data_saver, controller_vids, run_start_tick, dt, state)
...
def main():
    ...
    try:
        ...
        generate_trips_with_gaps(cosim_client, trip_specs, data_saver, controller_vids, run_start_tick, dt, state)
